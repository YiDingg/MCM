...
% 搜索 crush 时间
    ...
    t_step2 = 0.1;
    error_max_abs = 10^(-16);
    t_crushed = GetCrushedTime(b, d, v_0, t_range, t_step1, t_step2, error_max_abs);

% 输出结果
    theta_0 = 16*2*pi - func_find_start(b, 16*2*pi, v_0*(t_crushed-error_max_abs));
    theta_all = GetAllPoints(theta_0, b, d, 10^(-16));
    speed_all = GetAllSpeed(theta_all, b, v_0);
    PrintResult_Q2(theta_all, speed_all, b);   
    disp(['t_crushing = ', num2str(t_crushed, '%.12f')])
% 可靠性检验 (结果可视化)
    theta_0 = 16*2*pi - func_find_start(b, 16*2*pi, v_0*(t_crushed-t_step2));
    theta_all = GetAllPoints(theta_0, b, d, 10^(-16));
    stc = DrawPointsAndRectangles(theta_all, b);
