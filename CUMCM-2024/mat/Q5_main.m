...
% 根据此调头半径计算所需参数
    % 给定 R_turn_min
    R_t_min = 4.254674;
    global R_t; R_t = ceil(R_t_min*10^(8))/10^(8);
...
% 检查 R_turn_min 是否可行
    [logic_crush, logic_speederror, t_crushing] = CheckCrushAndSpeed_Q4([0, 50], 0.1);
...
v_0_range = [0.1, 2];
v0_error_max = 10^(-10);
v0_times_t_step = 0.05;  % 用于确定 t_range
v_0_max = GetMax_V0_Q5(v_0_range, v0_error_max, v0_times_t_step);

disp(['v_0_max = ', num2str(v_0_max, '%.12f')])

%% 问题五函数区 %%
%%%%%%%%%%%%%%%%%

function v_0_max = GetMax_V0_Q5(v_0_range, v0_error_max, v0_times_t_step)
    % 第一层步长: R_step1
    global v_0
    v_0_array = linspace(v_0_range(1), v_0_range(2), 6);
        for i = 1:length(v_0_array)
            v_0_new = v_0_array(i);
            t_range = [0, 40/v_0_new];
            t_step = v0_times_t_step/v_0_new;
            if t_step > 0.5
                t_step = 0.5;
            end
            v_max = GetVMax(v_0_new, t_range, t_step);
            disp(['v_max = ', num2str(v_max), ', v_0 = ', num2str(v_0) ]);
            if v_max > 2
                v_0_range = [v_0_array(i-1), v_0_array(i)];
                break
            end
        end
    ...
    % 第一层步长: R_step1
    v_0_array = linspace(v_0_range(1), v_0_range(2), 10);
        for i = 1:length(v_0_array)
            v_0_new = v_0_array(i);
            t_range = [0, 40/v_0_new];
            t_step = v0_times_t_step/v_0_new;
    ...
    for i = 1:71     % ceil( (log(4*pi)+20*log(10))/log(2) ) = 71, 这使得 4*pi/2^n < 10^(-20)
        ...
        temp = 0.5 * (max + min);
        t_range = [0, 40/temp];
        t_step = v0_times_t_step/temp;
        v_max = GetVMax(temp, t_range, t_step);
        if v_max < 2
            min = temp;
        else
            max = temp;
    ...
    % 输出结果
    v_0_max = 0.5*(min+max);
    disp(['v_max = ', num2str(v_max, '%.12f'), ', v_0 = ', num2str(v_0, '%.12f') ]);
end


function v_max = GetVMax(v_0_now, t_range, t_step)
    global v_0; v_0 = v_0_now;
    ChangeGlobalVaribles
    t_array = t_range(1):t_step:t_range(2);
    speed_matrix = zeros(224, length(t_array));
    for i = 1:length(t_array)
        t = t_array(i);
        angel_withflag_0 = GetHeadLocation(v_0, t);
        angle_withflag_all = GetAllPoints_Q4(angel_withflag_0, 10^(-12));
        speed_matrix(:, i) = GetAllSpeed_Q4(angle_withflag_all, v_0);
    end
    v_max = max(speed_matrix, [], "all");
end


function ChangeGlobalVaribles
% 引入全局变量
